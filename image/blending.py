...
def fix(backgroud, src_pos, trgt_pos, size, margin=5):
    ...
    src_mask = cv2.GaussianBlur(np.ones(src_roi.shape, src_roi.dtype) * 255, (5, 5), 0, 0)

    return cv2.seamlessClone(src_roi, backgroud, src_mask, trgt_pos, cv2.NORMAL_CLONE)
